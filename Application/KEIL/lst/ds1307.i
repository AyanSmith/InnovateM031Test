...
static void DS1307LowLevelInit(void)
{
 I2C_Open(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)), 100000);
 I2C_SetSlaveAddr(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)), 0, (0x68 ), 0);
...
void DS1307_SetClockHalt(uint8_t halt) {
 ...
 RVal= I2C_ReadByteOneReg(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)),(0x68 ), 0x00) & 0x7F;
 ch = ch | RVal;
 I2C_WriteByteOneReg(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)),(0x68 ), 0x00,ch);
}

uint8_t CheckRTC(void)
{
 uint8_t Val = I2C_ReadByteOneReg(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)),(0x68 ), 0x00);
 if(g_I2C_i32ErrCode == (-1L))
  return 0;
 else
  return 1;
}
...
_Bool DS1307_GetTime(_RTC *rtc)
{
  ...
  uint8_t buffer[7] = {0,};
 I2C_ReadMultiBytesOneReg(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)),(0x68 ), startAddr, buffer, 7);
...
_Bool DS1307_SetTime(_RTC *rtc)
{
  uint8_t startAddr = 0x00;
  uint8_t buffer[8];
 buffer[0] = D2B(rtc->Sec);
 buffer[1] = D2B(rtc->Min);
 buffer[2] = D2B(rtc->Hour);
 buffer[3] = D2B(rtc->DaysOfWeek);
 buffer[4] = D2B(rtc->Date);
 buffer[5] = D2B(rtc->Month);
 buffer[6] = D2B(rtc->Year);
 I2C_WriteMultiBytesOneReg(((I2C_T *) ((( uint32_t)0x40000000) + 0x80000)),(0x68 ), startAddr,buffer,7);
  return 1;
}
