# 1 "../Source/Utilities.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 349 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Source/Utilities.c" 2
...
# 11 "../Source/Utilities.c" 2
# 24 "../Source/Utilities.c"
void AppendVariableString(char* dest,char* source,uint8_t maxLength,uint8_t MinLength,char* defaultValue)
{
 uint16_t ln;
 ln = strlen(source);
...
void InsertStringValue(char* target,const char* value, uint16_t position, uint16_t length, uint8_t wh)
{
 ...
 if(ln > 0)
 {
  ...
  while(i<ln)
  {
   if(((* __rt_ctype_table())[value[i]] & (8 +16 +32 +2 +4)))
    vl=value[i];
   else
    vl='0';
   if((wh) && (vl=='-'))
