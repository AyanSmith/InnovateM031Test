...
typedef struct
{
 enum { UNKNOWN, MODEM_OK, SIM_DETECTED, INIT_GPRS, GPRS_ACTIVE, CONNECTED} GSMState;
...
extern GPSTypedef GPS;

extern NET_Typedef NetWork;


extern uint8_t IsServerData;
extern char ServerRxData[];

void M590Init(void);
void ProcessGPRS(void);
uint16_t SendDataToServer(char* data, char index);
void UpdateTimeFromGSM(void);
...
extern uint8_t FirmWareUpdated;
void LoadBootConfig(void);
void EraseNEWAPP(void);
void JumpToBOOT();
# 4 "../Source/M590.c" 2
# 1 "../Header\\FMCUser.h" 1
...
{
 uint16_t defID;
 ServerTypedef ServerData[2];
 ScheduleTypedef Schedule;
 ElectricTypedef RPhase;
 ElectricTypedef YPhase;
 ElectricTypedef BPhase;
 uint16_t Interval;
 uint8_t CCMSState;
 uint8_t CCMSControl;
 double Latitude;
...
extern LEDTypedef LedState;

inline static void InsertChar(char* s, char value)
{
    unsigned int i;
    i=strlen(s);
    s[i]=value;
...
void LED0_Set(uint16_t TotalTime, uint16_t OnTime);
void CCMSInit(void);
void LatchON(void);
void LatchOff(void);
void ProcessCCMS(void);
# 7 "../Header\\FMCUser.h" 2
...
char ModemRx[250];
char ModemCMD[150];
char ServerRxData[150];
uint16_t RxTail,SrTail;
uint8_t ServerStart;
char mAPN[20];
char gSignal[10];
char Mchar;

uint8_t IsSIM, IsSMS, IsModemInit, IsServerData,ServerCMD;



uint8_t PacketsRemaining;
uint16_t NoOfPackets;
uint16_t AppSize;
uint16_t CurrentPacket;
uint16_t FileIndex;

extern uint16_t ServerHangTimeOut;


 uint8_t PacketBuff[128u];
void FTPFileInfo(void);
uint8_t DownloadAPP2Flash(void);
void SetUpdateREQ(uint16_t size);
void UpdateNTP(void);
unsigned char WriteModem(char* data, char* response, unsigned long timeout);
...
static void M590LowLevelInit(void)
{
 SYS_ResetModule(((0x4<<24)|(18)));
 UART_Open(((UART_T *) ((( uint32_t)0x40000000) + 0x72000)), 115200);
...
void M590Init(void)
{
 ...
 GSM.GSMState=UNKNOWN;
 M590LowLevelInit();
 WriteModem("AT+CFUN=0\r\n","OK",5000);
 Delay(500);

 WriteModem("AT+CFUN=1\r\n","OK",2000);
 Delay(500);
...
void UART02_IRQHandler(void)
{
 ...
 if ((((((UART_T *) ((( uint32_t)0x40000000) + 0x72000)))->INTSTS & ((0x1ul << (8))))?1:0))
 {
  while((((((UART_T *) ((( uint32_t)0x40000000) + 0x72000)))->INTSTS & (0x1ul << (0)))>>(0)))
    {
   u8InChar = ((((UART_T *) ((( uint32_t)0x40000000) + 0x72000)))->DAT);
   ModemRx[RxTail++]=u8InChar;
   if(RxTail > (250 -5))
    RxTail = 0;

   if((ServerStart) && (!IsServerData))
   {
    if(u8InChar == '\n')
    {
     ServerRxData[SrTail] = 0;
     ServerStart=0;
     IsServerData = 1;
    }
    else
     ServerRxData[SrTail++] = u8InChar;
   }
   else
   {
    if(strstr(ModemRx,"RECV:"))
    {
     SrTail = 0;
     ServerStart = 1;
    }
...
void DefaulParam(uint8_t bl)
{
 GSM.MCC=404;
 GSM.MNC= 564;
 strcpy(GSM.LAC,"000");
 strcpy(GSM.CellID,"0000");
 if(bl)
 {
  memset(GSM.LAC,0x00,6);
  memset(GSM.CellID,0x00,7);
 }
}

uint16_t CheckModemRX(void)
{
 if(strstr(ModemRx,"+CMT"))
 {
  IsSMS = 1;
  return 1;
 }
...
void ClearModemBuffer(void)
{
 CheckModemRX();
 memset(ModemRx,0x00,250);
 RxTail=0;
}

void SendDataToModem(char* data)
{
 while(*data)
 {

  ((((UART_T *) ((( uint32_t)0x40000000) + 0x72000)))->DAT = (*data));
...
unsigned char WriteModem(char* data, char* response, unsigned long timeout)
{
 unsigned int cycle = 10;
  volatile uint32_t received_char;
 ...
 while(cycle > 0)
 {
  if(strstr(ModemRx,response))
  {
   return 1;
  }
  if(strstr(ModemRx,"ERROR"))
   return 0;
...
uint16_t TestModem(void)
{
 memset(NetWork.SIMNo,0X00,20);
 memset(NetWork.IMEI,0X00,18);
 WriteModem("ATE0\r\n","OK",500);
 Delay(50);
 ...
 if(WriteModem("AT\r\n","OK",1000))
 {
  ...
  WriteModem("ATE0\r\n","OK",1000);
  Delay(50);
  WriteModem("AT+CMGF=1\r\n","OK",1000);





  WriteModem("AT+CNMI=2,1\r\n","OK",1000);
  WriteModem("AT+GSN\r\n","OK",3000);
  if(strstr(ModemRx,"ERROR"))
  {
    GSM.GSMState=MODEM_OK;
    return 1;
  }
  fn = strstr(ModemRx, "N: ");
  if(fn)
   strncpy(NetWork.IMEI,&fn[3],15);
...
unsigned char GetSignal(void)
{
  char* fn;
  char* ls;
  unsigned char sg;
 memset(gSignal,0x00,10);
 WriteModem("AT\r\n","OK",2000);
 if(strlen(ModemRx) == 0)
  Delay(4000);
  WriteModem("AT+CSQ\r\n","OK",2000);
  fn=strstr(ModemRx,"+CSQ:");
  ls=strstr(fn,",");
  if((fn) && (ls))
  {
  ls[0]=0;
    sg=atoi(&fn[6]);
  strcpy(gSignal,&fn[6]);
  GSM.SignalStrength = sg;
    return sg;
  }
  return 0;
}

void DetectSIMCard(void)
{
 char* fn;
 WriteModem("ATE0\r\n","OK",1000);
 Delay(50);
 if(WriteModem("AT+CPIN?\r\n","READY",1000))
 {
  IsSIM=1;
  WriteModem("AT+CCID\r\n,","OK",1000);
  fn = strstr(ModemRx,"+CCID:");
  if(strstr(ModemRx,"ERROR"))
   memset(NetWork.SIMNo,0X00,20);
  else if(fn)
   strncpy(NetWork.SIMNo,&fn[6],21);
...
uint8_t GetCREG(void)
{
 char* fn;
 WriteModem("AT+CREG?\r\n","OK",2000);
 fn=strstr(ModemRx,"REG:");
 if(fn)
 {
  if((fn[7] == '1') || (fn[7] == '5'))
   return 1;
...
void GetSPName(void)
{
 ...
 unsigned char len;
 WriteModem("AT\r\n","OK",2000);
 WriteModem("AT+COPS=0,0\r\n","OK",200);

 if(WriteModem("AT+COPS?\r\n","OK",3000))
 {
  p=strchr(ModemRx, '"');
  if(p)
  {
   ...
   len = (f-p) - 1;
   if(len < 40)
    strncpy(NetWork.Network, &p[1], len);
  }
 }
}

void AutoUpdateAPN(void)
{
 memset(NetWork.APN,0x00,12);

 if(strstr(NetWork.Network,"Airtel") || strstr(NetWork.Network,"airtel") )
  strcpy(NetWork.APN,"airtelgprs.com");
 else if(strstr(NetWork.Network,"Jio") || strstr(NetWork.Network,"jio")
  || strstr(NetWork.Network,"JIO"))
  strcpy(NetWork.APN,"JioNet");
 else
  strcpy(NetWork.APN,"airtelgprs.com");
}

uint16_t InitGPRS(void)
{
 GSM.SignalStrength = GetSignal();
 ...
 if(GetCREG())
 {
  ...
  GetSPName();
  AutoUpdateAPN();
  WriteModem("AT+CGDCONT?\r\n","OK",1000);
  if(!strstr(ModemRx,NetWork.APN))
  {
   memset(ModemCMD, 0x00, 50);
   strcpy(ModemCMD,"AT+CGDCONT=1,\"IP\",\"");
   strcat(ModemCMD, NetWork.APN);
   strcat(ModemCMD,"\"\r\n");
   if(WriteModem(ModemCMD,"OK", 2500))
   {
    GSM.GSMState=INIT_GPRS;
...
uint16_t ActivateGPRS(void)
{
 WriteModem("AT+XIIC?\r\n","OK",1500);
 if(!strstr(ModemRx,"1,"))
 {
  if(WriteModem("AT+XIIC=1\r\n","OK",2500))
  {
   Delay(500);
   WriteModem("AT+XIIC?\r\n","OK",1500);
   if(strstr(ModemRx,"1,"))
...
uint16_t ConnectToServer(uint8_t index)
{
 memset(ModemCMD,0x00,50);

 ActivateGPRS();
 GSM.SignalStrength = GetSignal();
 strcpy(ModemCMD,"AT+IPSTATUS=0\r\n");
 ModemCMD[12]=index+'0';
 if(WriteModem(ModemCMD,"CONNECTING",1000)==1)
  return 0;
 memset(ModemCMD,0x00,50);
 strcpy(ModemCMD, "0,CONNECT");
 ModemCMD[0]=index + '0';
 if(strstr(ModemRx,ModemCMD))
 {
  GSM.GSMState=CONNECTED;
  return 1;
 }
 memset(ModemCMD,0x00,50);

 strcpy(ModemCMD, "AT+TCPSETUP=0,");
 ModemCMD[12]=index+'0';
 if(index == 0)
  strcat(ModemCMD,CCMS.ServerData[0].ServerIP);
 else
  strcat(ModemCMD,CCMS.ServerData[1].ServerIP);
 strcat(ModemCMD,",");

 if(index == 0)
  strcat(ModemCMD,CCMS.ServerData[0].PortNo);
 else
  strcat(ModemCMD,CCMS.ServerData[1].PortNo);

 strcat(ModemCMD,"\r\n");

 if(index==0)
 {
  if(WriteModem(ModemCMD,"TCPSETUP:0,OK",3500))
  {
   GSM.GSMState=CONNECTED;
   return 1;
  }
 ...
 {
  if(WriteModem(ModemCMD,"TCPSETUP:1,OK",3500))
  {
   GSM.GSMState=CONNECTED;
   return 1;
  }
...
uint32_t GetSendAck(char index)
{
 char ss[25];
 char *fn;
 char *ls;
 ...
 strcpy(ss,"AT+TCPACK=");
 InsertChar(ss,index + '0');
 strcat(ss,"\r\n");
 if(WriteModem(ss,"ACK:",1000))
 {
  fn=strstr(ModemRx,"CK:");
  if(fn)
  {
   ls = strchr(ModemRx,',');
   if(ls)
...
uint8_t WaitAck(char index)
{
 uint32_t ack, sz;
 uint16_t i;
 for(i=0;i<30;i++)
 {
  ack=GetSendAck(index);
  if(!ack)
  {
   if(strstr(ModemRx,"DISCONNECT"))
    return 0;
...
uint16_t SendDataToServer(char* data, char index)
{
 ...
 GSM.SignalStrength = GetSignal();
 creg = GetCREG();
 if((GSM.SignalStrength < 9) || (creg ==0))
 {
  GSM.GSMState= SIM_DETECTED;
  return 0;
 }

 if(ConnectToServer(index) == 0)
 {
  GSM.GSMState = GPRS_ACTIVE;
  return 0;
 }
 ...
 memset(ModemCMD,0x00,50);
 if(!index)
  WriteModem("AT+IPSTATUS=0\r\n","OK",1000);
 else
  WriteModem("AT+IPSTATUS=1\r\n","OK",1000);
 strcpy(ModemCMD,"AT+TCPSEND =0,");
 ModemCMD[12] = index + '0';
 strcat(ModemCMD,ss);
 strcat(ModemCMD,"\r\n");
 if(WriteModem(ModemCMD,">",500))
 {
  SendDataToModem(data);
  Delay(500);
 }
 if(strstr(ModemRx,"OK"))
 {

  if(WaitAck(index) == 0)
  {
   WriteModem("AT+TCPCLOSE=0\r\n","OK",1000);
   M590Init();
...
void UpdateNTP(void)
{
 ...
 if(GSM.GSMState < GPRS_ACTIVE)
  return;
 WriteModem("AT+UPDATETIME=1,128.138.141.172,10,\"E5:30\",0\r\n","iat",2000);
 Delay(200);
}
...
void UpdateTimeFromGSM(void)
{
 char * fn;
 char ss[10];
 uint32_t unixGSM,unixRTC;
 int diff;
 WriteModem("AT+CCLK?\r\n","OK",1000);
 fn=strstr(ModemRx,"LK:");
 if(fn)
 {
  strncpy(ss,&fn[11],2);
  CurrentDateTime.Date = atoi(ss);

  strncpy(ss,&fn[8],2);
  CurrentDateTime.Month = atoi(ss);

  strncpy(ss,&fn[5],2);
  CurrentDateTime.Year = atoi(ss);

  strncpy(ss,&fn[14],2);
  CurrentDateTime.Hour = atoi(ss);

  strncpy(ss,&fn[17],2);
  CurrentDateTime.Min = atoi(ss);

  strncpy(ss,&fn[20],2);
  CurrentDateTime.Sec = atoi(ss);
  if(CurrentDateTime.Year > 22)
  {
   ...
   if(RTCDateTime.Year != CurrentDateTime.Year)
   {
    DS1307_SetTime(&CurrentDateTime);
    IsTimeSet=1;
    return;
   }
   else
   {
    unixGSM = (CurrentDateTime.Month*30*24*60)+(CurrentDateTime.Date*24*60)+(CurrentDateTime.Hour*60);
    unixRTC = (RTCDateTime.Month*30*24*60)+(RTCDateTime.Date*24*60)+(RTCDateTime.Hour*60);
    diff = (int)unixGSM - unixRTC;
    if((diff > 2) || (diff < -2))
    {
     DS1307_SetTime(&CurrentDateTime);
     IsTimeSet=1;
...
void SetUpdateREQ(uint16_t Appsize)
{
 ...
 SYS_UnlockReg();
 FMC_Open();
 (((FMC_T *) ((( uint32_t)0x40000000) + 0x0C000))->ISPCTL |= (0x1ul << (5)));
 Fsize = sizeof(BootConfig);
 ReadFMCDataBuffer((uint8_t*)&BootConfig,0x400UL +0x00100000UL,Fsize);
 if(BootConfig.DefValue != 0x5A3A)
  return;
 BootConfig.UpdateREQCode = 0xA5A3;
 BootConfig.NewAppSize = Appsize;
 EraseFMCData(0x400UL +0x00100000UL,Fsize);
 WriteFMCData((uint8_t*)&BootConfig,0x400UL +0x00100000UL,Fsize);
 (((FMC_T *) ((( uint32_t)0x40000000) + 0x0C000))->ISPCTL &= ~(0x1ul << (5)));
 FMC_Close();
 SYS_LockReg();
...
uint8_t GetFTPStatus(void)
{
 WriteModem("AT+FTPSTATUS\r\n","US:",16000);
 if(strstr(ModemRx,"1,"))
 {
  return 1;
 }
...
uint8_t FTPLogin(void)
{
 memset(ModemCMD,0x00,65);
 WriteModem("AT+TCPCLOSE=0\r\n","OK",2000);
 Delay(200);
 if(GetFTPStatus())
 {
  WriteModem("AT+FTPLOGOUT\r\n","CME",16000);
 }
 strcpy(ModemCMD,"AT+FTPLOGIN=");
 strcat(ModemCMD,CCMS.ServerData[0].FTPSite);
 InsertChar(ModemCMD,',');
 strcat(ModemCMD,CCMS.ServerData[0].FTPPort);
 InsertChar(ModemCMD,',');
 strcat(ModemCMD,CCMS.ServerData[0].FTPUser);
 InsertChar(ModemCMD,',');
 strcat(ModemCMD,CCMS.ServerData[0].FTPPass);
 strcat(ModemCMD,"\r\n");
 WriteModem(ModemCMD,"GIN:",16000);

 Delay(2000);
 if(strstr(ModemRx,"logged") || strstr(ModemRx,"Logged"))
 {
  if(strstr(ModemRx,"Not"))
   return 0;
  GSM.GSMState = CONNECTED;
  if(DownloadAPP2Flash())
   return 1;
 }
 return 0;
}
...
void FTPFileInfo(void)
{
 ...
 WriteModem("AT+FTPGET=Hex/CCMSF.bin,2,1,0,32\r\n","ET:",30000);
 if(strstr(ModemRx,"ERROR"))
  GSM.GSMState = GPRS_ACTIVE;
...
uint8_t DownloadAPP2Flash(void)
{
 ...
 WriteModem("AT+FTPSIZE=Hex/CCMSF.bin\r\n","ZE:",3000);
 Delay(10);
 fn = strstr(ModemRx,"ZE:");
 if(!(fn))
  return 0;
 fn = &fn[3];
 if(fn[0] == ' ')
 ...
 NoOfPackets = (AppSize / 128u);
 PacketsRemaining = AppSize % 128u;
 ...
 SYS_UnlockReg();
 FMC_Open();
 (((FMC_T *) ((( uint32_t)0x40000000) + 0x0C000))->ISPCTL |= (0x1ul << (3)));
 for(CurrentPacket=0;CurrentPacket<NoOfPackets;CurrentPacket++)
 {
  strcpy(ModemCMD,"AT+FTPGET=Hex/CCMSF.bin,2,1,");
  sprintf(ss,"%u",FileIndex);
  strcat(ModemCMD,ss);
  strcat(ModemCMD,",128\r\n");


  WriteModem(ModemCMD,"ET:",30000);
  Delay(20);
  if(strstr(ModemRx,"ERROR"))
   return 0;
  fn = strchr(ModemRx,',');
  if(fn++)
  {
   ...
   WriteFMCData((uint8_t*)fn,(FileIndex + 0x14000UL),0x80);
   ReadFMCDataBuffer(PacketBuff,(FileIndex + 0x14000UL),0x80);
   Delay(10);
  }
  else
   return 0;
