...
typedef struct
{
 ...
 uint16_t Freq;
 uint8_t IsMeter;
 enum {EMETER=0,MAXWELL=1} MeterType;
}METER_PARAM;
...
# 168 "C:\\Keil_v5\\ARM\\ARMCLANG\\Bin\\..\\include\\ctype.h" 3
    extern int (isdigit)(int );



    extern int (isblank)(int );
# 181 "C:\\Keil_v5\\ARM\\ARMCLANG\\Bin\\..\\include\\ctype.h" 3
...
# 206 "C:\\Keil_v5\\ARM\\ARMCLANG\\Bin\\..\\include\\ctype.h" 3
...
# 16 "../Source/Server.c" 2



uint16_t IsDataReady;
uint8_t IsStored = 0;
uint8_t currentPhase=0;
uint8_t PacketCount = 0;


void DecodeServerData(void);

char dataBuffer[512];
extern volatile uint16_t ServerHangTimeOut;


extern void UpdateConfigInFlash(void);

void MakeDataString(void)
{
 char temp[20];
 char phase[3] = {'R','Y','B'};
 if(currentPhase > 2)
 ...
 strcpy(dataBuffer,"$AN,");
 strcat(dataBuffer,NetWork.IMEI);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%0.2d%0.2d%0.2d", CurrentDateTime.Hour,CurrentDateTime.Min,CurrentDateTime.Sec);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 memset(temp,0x00,20);
 sprintf(temp,"%0.2d%0.2d%0.2d", CurrentDateTime.Date,CurrentDateTime.Month,CurrentDateTime.Year);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 memset(temp,0x00,20);
 sprintf(temp,"%i", GSM.SignalStrength);
 strcat(dataBuffer, temp);
 InsertChar(dataBuffer,',');
 memset(temp,0x00,20);
 switch(currentPhase)
 {
  case 0:
   sprintf(temp,"%i", MeterParam.RPhase.Volt);
   strcat(dataBuffer, temp);
   InsertChar(dataBuffer, ',');
   sprintf(temp,"%i",MeterParam.RPhase.Amp);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   sprintf(temp,"%i",MeterParam.RPhase.mPF);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');

   sprintf(temp,"%i",MeterParam.RPhase.kW);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   break;
  case 1:
   sprintf(temp,"%i", MeterParam.YPhase.Volt);
   strcat(dataBuffer, temp);
   InsertChar(dataBuffer, ',');
   sprintf(temp,"%i",MeterParam.YPhase.Amp);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   sprintf(temp,"%i",MeterParam.YPhase.mPF);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');

   sprintf(temp,"%i",MeterParam.YPhase.kW);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   break;
  case 2:
   sprintf(temp,"%i", MeterParam.BPhase.Volt);
   strcat(dataBuffer, temp);
   InsertChar(dataBuffer, ',');
   sprintf(temp,"%i",MeterParam.BPhase.Amp);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   sprintf(temp,"%i",MeterParam.BPhase.mPF);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');

   sprintf(temp,"%i",MeterParam.BPhase.kW);
   strcat(dataBuffer,temp);
   InsertChar(dataBuffer,',');
   break;
 ...
 sprintf(temp,"%i",MeterParam.TotalKWH);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');

 sprintf(temp,"%i",MeterParam.TotalKVAH);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%i",CCMS.CCMSState);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%i",CCMS.CCMSControl);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 InsertChar(dataBuffer,phase[currentPhase]);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%02i:%02i",CCMS.Schedule.OnTime.Hour,CCMS.Schedule.OnTime.Min);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%02i:%02i",CCMS.Schedule.OffTime.Hour,CCMS.Schedule.OffTime.Min);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 sprintf(temp,"%i",PacketCount);
 strcat(dataBuffer,temp);
 InsertChar(dataBuffer,',');
 strcat(dataBuffer,"AN240130#");
 currentPhase++;
}

void MakeBLESystemString(void)
{
 char ss[50];
 memset(dataBuffer,0x00,512);
 ...
 if(!MeterParam.IsMeter)
  strcat(dataBuffer,"Meter NOT Detected\n");
 else
 {
  if(MeterParam.MeterType==MAXWELL)
   strcat(dataBuffer,"MAXWELL Meter Detected\n");
  else
   strcat(dataBuffer,"EppleTone Meter Detected\n");
 }
 if(IsRTC)
 {
  strcat(dataBuffer,"RTC Ok\n");
  sprintf(ss,"Date: %02d/%02d/%02d\nTime: %02d:%02d:%02d\n",
  RTCDateTime.Date,RTCDateTime.Month,RTCDateTime.Year,
  RTCDateTime.Hour,RTCDateTime.Min,RTCDateTime.Sec);
 }
 else
 {
  strcat(dataBuffer,"RTC Err\n");
  sprintf(ss,"Date: %02d/%02d/%02d\nTime: %02d:%02d:%02d\n",
 ...
 if(IsTimeSet)
  strcat(dataBuffer,"Time Set\n");
 else
  strcat(dataBuffer,"Time Not Set\n");

 strcat(dataBuffer,ss);


 if(GSM.GSMState > MODEM_OK)
  strcat(dataBuffer,"SIM Detected\n");
 else
  strcat(dataBuffer,"SIM NOT Detected\n");
 ...
 if(GSM.GSMState >= GPRS_ACTIVE)
  strcat(dataBuffer,"GPRS Active\n");
 else
  strcat(dataBuffer,"GPRS NOT Active\n");

 if(GSM.GSMState >= CONNECTED)
  strcat(dataBuffer,"SERVER Connected\n");
 else
  strcat(dataBuffer,"SERVER Not Connected\n");

 if(CCMS.CCMSState & 0x01)
  strcat(dataBuffer,"RELAY ON\n");
 else
  strcat(dataBuffer,"RELAY OFF\n");

 sprintf(ss,"Stat Byte : %03d\nControl Byte: %03d\n",CCMS.CCMSState,CCMS.CCMSControl);
 strcat(dataBuffer,ss);
...
void DecodeServerData(void)
{
 ...
 if(strstr(ServerRxData,"F8,1,"))
 {
  LatchON();
  CCMS.CCMSControl |= 0x04;
  CCMS.CCMSState |= 0x04;
 ...
 if(strstr(ServerRxData,"SI,"))
 {
  fn = strstr(ServerRxData,"I,");
  fn+=2;
  ls = strstr(ServerRxData,",");
  if(ls)
  {
   ls[0] = 0;
   n = atoi(fn);
 ...
 if((strstr(ServerRxData, "L010")) || (strstr(ServerRxData,"L8,1,")))
 {

  if(CCMS.CCMSControl & 0x04)
  {
   CCMS.CCMSState |= 0x04;
   (*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((4)<<2))))=1;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((3)<<2))))=1;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((2)<<2))))=1;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((5)<<2))))=1;CCMS.CCMSState|=0x01;
   LatchON();
  }
  return;
 }

 if((strstr(ServerRxData, "L000")) || (strstr(ServerRxData,"L8,2,")))
 {

  if(CCMS.CCMSControl & 0x04)
  {
   (*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((4)<<2))))=0;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((3)<<2))))=0;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((2)<<2))))=0;(*((volatile uint32_t *)(((((( uint32_t)0x40000000) + 0x4000) + 0x0800)+(0x40*(2))) + ((5)<<2))))=0;CCMS.CCMSState&=(~0x01);
   LatchOff();
   CCMS.CCMSState &= ~0x04;
  }
  return;
 ...
 if((strstr(ServerRxData, "L000")) || (strstr(ServerRxData,"L2,")))
 {
  fn=strstr(ServerRxData,"L2,");
  if(fn)
  {
   n=((fn[3] - '0') * 10) + (fn[4] - '0');
   CCMS.Schedule.OnTime.Hour=n;
   n=((fn[6] - '0') * 10) + (fn[7] - '0');
   CCMS.Schedule.OnTime.Min=n;

   n=((fn[9] - '0') * 10) + (fn[10] - '0');
   CCMS.Schedule.OffTime.Hour=n;
   n=((fn[12] - '0') * 10) + (fn[13] - '0');
   CCMS.Schedule.OffTime.Min=n;
   UpdateConfigInFlash();
   return;
  }
 ...
 if((strstr(ServerRxData, "L000")) || (strstr(ServerRxData,"LS,")))
 {
  fn=strstr(ServerRxData,"LS,");
  if(fn)
  {
   n=((fn[3] - '0') * 100) + ((fn[4] - '0')* 10) + (fn[5] - '0');
 ...
 if((strstr(ServerRxData, "L000")) || (strstr(ServerRxData,"LI,")))
 {
  fn=strstr(ServerRxData,"LI,");
  if(fn)
  {
   ls=strchr(fn,',');
   if(ls)
   {
    ls[0]=0;
    strcpy(CCMS.ServerData[0].ServerIP,&fn[3]);
    ls++;
    fn=strchr(ls,',');
    if(fn)
    {
     fn[0]=0;
     strcpy(CCMS.ServerData[0].PortNo,ls);
    }
    UpdateConfigInFlash();
    return;
   }
 ...
 if((strstr(ServerRxData, "L000")) || (strstr(ServerRxData,"LR,")))
 {
  fn=strstr(ServerRxData,"LR,1,");
  if(fn)
  {
   CCMS.CCMSControl |= 0x02;
   UpdateConfigInFlash();
   return;
  }
  fn=strstr(ServerRxData,"LR,0,");
  if(fn)
  {
   CCMS.CCMSControl &= ~0x02;
   UpdateConfigInFlash();
 ...
 if(strstr(ServerRxData, "RST"))
 {
  fn=strstr(ServerRxData,"RST,1");
  if(fn)
  {
   M590Init();
   return;
  }
  fn=strstr(ServerRxData,"RST,0");
  if(fn)
  {
   NVIC_SystemReset();
   return;
