# 1 "../Source/NMEAParser.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 349 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Source/NMEAParser.c" 2

# 1 "../Header\\NMEAParser.h" 1
...
_Bool NMEA_Parse_RMC( RMCTypedef *frame, const char *sentence);
_Bool NMEA_Parse_VTG( VTGTypedef *frame, const char *sentence);
_Bool NMEA_Parse_GGA(GGATypedef *frame, const char *sentence);
# 3 "../Source/NMEAParser.c" 2
...
# 6 "../Source/NMEAParser.c" 2




static _Bool NMEA_Isfield(char c) {
    return ((* __rt_ctype_table())[(unsigned char) c] & (8 +16 +32 +2 +4)) && c != ',' && c != '*';
}
...
_Bool NMEA_Check(const char *sentence, _Bool strict)
{
    uint8_t checksum = 0x00;
  int upper, lower, expected;

    if (strlen(sentence) > 100 + 3)
        return 0;
    ...
    while (*sentence && *sentence != '*' && ((* __rt_ctype_table())[(unsigned char) *sentence] & (8 +16 +32 +2 +4)))
        checksum ^= *sentence++;
...
_Bool NMEA_Scan(const char *sentence, const char *format, ...)
{
 _Bool result = 0;
 _Bool optional = 0;
    ...
    while (*format) {
        ...
        switch (type) {
            case 'c': {
                ...
                *__builtin_va_arg(ap, char *) = value;
            } break;
            ...
            case 'f': {
                ...
                if (field) {
                    while (NMEA_Isfield(*field)) {
                        if (*field == '+' && !sign && value == -1) {
                            ...
                            sign = -1;
                        } else if (((* __rt_ctype_table())[(unsigned char) *field] == 32)) {
                            int digit = *field - '0';
                            if (value == -1)
                                value = 0;
                ...
                *__builtin_va_arg(ap, struct NMEA_FLOAT *) = (struct NMEA_FLOAT) {value, scale};
       ...
       int f;
                ...
                struct NMEA_DATE *date = __builtin_va_arg(ap, struct NMEA_DATE *);
                ...
                if (field && NMEA_Isfield(*field)) {
                    ...
                    for (f=0; f<6; f++)
                        if (!((* __rt_ctype_table())[(unsigned char) field[f]] == 32))
                            goto parse_error;
            ...
            case 'T': {
                struct NMEA_TIME *time_ = __builtin_va_arg(ap, struct NMEA_TIME *);
                ...
                if (field && NMEA_Isfield(*field)) {
                    ...
                    if (*field++ == '.') {
                        ...
                        while (((* __rt_ctype_table())[(unsigned char) *field] == 32) && scale > 1) {
        ...
        do { while (NMEA_Isfield(*sentence)) sentence++; if (*sentence == ',') { sentence++; field = sentence; } else { field = 0; } } while (0);
    }
...
_Bool minmea_talker_id(char talker[3], const char *sentence)
{
    char type[6];
...
enum NMEA_SENTENCE_ID nmea_sentence_id(const char *sentence, _Bool strict)
{
...
_Bool NMEA_Parse_GSA(GSATypedef *frame, const char *sentence)
{
...
_Bool NMEA_Parse_GLL(GLLTypedef *frame, const char *sentence)
{
...
_Bool NMEA_Parse_GST(GSTTypedef *frame, const char *sentence)
{
...
_Bool NMEA_Parse_GSV(GSVTypedef *frame, const char *sentence)
{
...
_Bool NMEA_Parse_ZDA( ZDATypedef *frame, const char *sentence)
{
...
int NMEA_GetTime(struct tm *ts, const struct NMEA_DATE *date, const struct NMEA_TIME *time_)
{
